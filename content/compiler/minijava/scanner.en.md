 ...
 * The regular expression $\color{green}\epsilon$ generates the set containing only the empty word[^1]: $\\{\epsilon\\}$.
 ...
 * The iteration operator noted `*` allows to juxtapose $0$ or several times the words generated by a regular expression. Let $\color{green}{r}$ be a regular expression, then
 the regular expression $\color{green}{r^*}$ represents the hypothetical[^2] regular expression
...
But you may be a [Perl](https://fr.wikipedia.org/wiki/Perl_(language))[^4] guru
...
{{< figure src="images/minijava/scanner/dfa_to_regex_q2_6.en.svg" width="600px" height="auto">}}
...
the lexical analyzer, we obtain the following tokens[^5].
...
[^1]: The empty word is the equivalent of the string `""`.
[^2]: A regular expression must be of finite size.
[^3]: `ndf` for *non-deterministic finite*.
[^4]: `Perl` stands for *Practical Extraction and Report Language*, or *Pathologically Eclectic Rubbish Lister* <i class="far fa-smile-wink"></i>.
[^5]: More precisely, we obtain a representation of the tokens.